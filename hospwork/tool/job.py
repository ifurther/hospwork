...
def findjobtype(job_type,name)-> str:
    '''find the job type'''
    job_type_pattern = r"\B[院,處,行,課,科,短,社,契,牙,秘,醫,部,室,門,招,募,誠,聘](.*)[員,理,工,師,生,廚,士]"
    if '研究助理' in job_type:
        return '研究助理'
    elif '工讀生' in job_type:
        return '工讀生'
    try:
        return re.search(job_type_pattern,job_type).groups()[0].replace("招募","").replace('募','').replace("聘","")
    except AttributeError:
        print("{} Error find job type: {}".format(name, job_type))
        return job_type
def findjoboriginzation(job_type,name) -> str:
