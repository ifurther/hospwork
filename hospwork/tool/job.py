...
def findjobtype(job_type,name)-> str:
    '''find the job type'''
    try:
        return re.search(r"\B[科,短,社,契,牙,秘,醫,部,室,門,招,募,誠,聘](.*)[員,理,工,師,生,廚]",job_type).group(0).replace("招募","").replace('募','').replace("聘","")
...
def findjoboriginzation(job_type,name):
    ...
    try:
        return re.search(r"([秘,核,運,腎,心,生,婦,神,5,管,新,教,影,癌,血,牙,腦,骨,巨,工,傳,耳,放,復,營,資,護,家,胸,皮,泌,眼,外,藥,麻,臨,急,病,兒,環,健](\w+||)[科,部,室,心,課,處,房,局,組])|([內,中](\w+||)[科,局])",job_type.replace(name,"")).group(0)
    except AttributeError:
