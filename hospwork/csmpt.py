...
class Csmpt(Hospital_work):
    def __init__(self):
        ...
        self.pages_link = self.get_pages_link(self.work_page_base, self.url_full)

        work_table=[]
        work_table = self.get_work_table(self.work_page_base.find_all('article')[0],self.url_base,work_table)
        for p_i, p_item in enumerate(self.pages_link):
            table_ = get_work_page(p_item).find_all('article')[0]

            work_table = self.get_work_table(table_,self.url_base,work_table)

        self.work_table=pd.DataFrame(work_table, columns=['no','召聘職稱','期限' ,'連結'])

    def get_pages_link(self,soup,url):
        pages_link=[]
    ...
    def get_work_table(self,table_,url_base,work_table):
